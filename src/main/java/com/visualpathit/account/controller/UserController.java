...
public class UserController {
    ...
    // public final String registration(...)：这个方法接收多个参数，包括一个@ModelAttribute("userForm") User userForm，它绑定了表单提交的数据到一个User对象；一个BindingResult bindingResult，用于存储表单验证的结果；以及一个Model model，用于添加或修改模型属性。
    public final String registration(final @ModelAttribute("userForm") User userForm,final BindingResult bindingResult, final Model model) {
    ...
    public final String userUpdateProfile(@PathVariable(value = "username") String username,
            final @ModelAttribute("user") User userForm, final Model model) {
        User user = userService.findByUsername(username);
